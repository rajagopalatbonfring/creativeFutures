...
// import { ageGroupData, getEligibleActivities } from '@/lib/constants';
// import { Button } from '@/components/ui/button';
// import { ArrowRight, ArrowLeft, Sparkles, Zap } from 'lucide-react';
// import Image from 'next/image';
// import { PlaceHolderImages } from '@/lib/placeholder-images';

// export default async function AgeGroupDetailPage({ params }: { params: Promise<{ age: string }> }) {
//   const { age } = await params;

//   const group = ageGroupData.find(g => g.age === age);
//   if (!group) {
//     notFound();
//   }

//   const eligible = getEligibleActivities(age as any);
...
//                           }`}
//                         >
//                           <Link href={`/submit?activity=${activity.id}`} className="flex items-center justify-center gap-1">
//                             {isSpecial ? 'Join' : 'Start'}
...
//                 <Button asChild size="lg" className="bg-gradient-to-r from-[#FB0091] to-pink-500 hover:from-pink-500 hover:to-[#FB0091] text-white shadow-lg rounded-full font-bold px-8">
//                   <Link href="/gallery">
//                     View Gallery
//                   </Link>
//                 </Button>
...
// // app/participant-category/[age]/page.tsx
// // SERVER COMPONENT – keep 'use client' REMOVED
...
// app/participant-category/[age]/page.tsx
import { notFound } from 'next/navigation';
import { AgeGroupDetailClient } from '@/app/participant-category/age-group-detail-client';
import { ageGroupData, getEligibleActivities, isValidAgeGroup } from '@/lib/constants';

export default async function AgeGroupDetailPage({ params }: { params: Promise<{ age: string }> }) {
  const { age } = await params;

  // Runtime + type narrowing
  if (!isValidAgeGroup(age)) {
    notFound();
  }
  ...
  const eligible = getEligibleActivities(age); // now age is AgeGroup – no error

  return <AgeGroupDetailClient group={group} eligible={eligible} />;
}